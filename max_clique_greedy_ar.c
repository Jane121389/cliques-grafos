...
int greedy(int div, int v, struct grafica gr[], unsigned long Cl_uno[])
{
    int           k, i, n, h, j;
    unsigned long ColorClass[50][50], Cl[50];
    ...
    for (j=0; j < div; j++)
    {
        ...
        for (i=0; i < n; i++)
        {
            ...
            if (Cl[j] & uno)
            {
                while (h < k && ((gr[i + (j * 32)].conexiones[j] & Cl_uno[j] & ColorClass[j][h]) != 0)) {
                    h=h + 1;
                }
                if (h == k)
                {
                ...
                ColorClass[j][h]=ColorClass[j][h] | var;
                //printf(" h:%d- %d ",h,i);
                var2                =var2 << h;
                col[i + (j * 32)][j]=col[i + (j * 32)][j] | var2;
            }
            Cl[j]=Cl[j] >> 1;
            var  =var << 1;
        }
...
void backtrack(int n, int l, struct grafica gr[], unsigned long C[], unsigned long N[], unsigned long b[], int c, int div)
{
    ...
    for (i=0; i < div; i++)
    {
        ...
        while (m <= n_ver)
        {
            ...
            if (C1_aux[i] & uno)
            {
                ...
                backtrack(n, (m + (i * 32)), gr, C1, N_aux, b_aux, c + 1, div);
            }
            C1_aux[i]=C1_aux[i] >> 1;
            b_aux[i] =b_aux[i] << 1;
            b_aux[i] =b_aux[i] & (C0[i] - 1);
...
int main()
{
    ...
    while (l < v)
    {
        ...
        b[i]       =b[i] & C0[i];
        clique_m[0]=l;
        backtrack(v, l, nueva_g, C0, N, b, 1, div);
        l++;
    }
    printf("\nmaximo clique:");
    for (mc=0; mc < max; mc++)
